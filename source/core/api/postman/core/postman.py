...
class Request(object):
    ...
    @property
    def raw_mode_data(self):
        data = None
        if self.method not in ('POST', 'PUT'):
            return None
        # Add all parameters body
        for param in self.operation.get('parameters', []):
            if param['in'] == 'body':
                if param['name'] == 'payload' and '$ref' in param['schema']:
                    data = {}
                    uri = param['schema']['$ref'].split('/')[1:]
                    value = self.collection.api.__schema__
                    for u in uri:
                        if value.get(u):
                            value = value[u]
                    if value and 'properties' in value and isinstance(value['properties'], dict):
                        properties = value['properties']
                        for key in properties.keys():
                            if properties[key].get('items') and properties[key]['items'].get('type'):
                                data[key] = properties[key]['items']['type']
                            else:
                                data[key] = properties[key].get('type')
    ...
    def as_dict(self, urlvars=False):
        ...
        return clean({
            ...
            'data': self.data,
            'dataMode': "raw" if self.raw_mode_data else "params" if self.data else None,
            'queryParams': self.query_params,
            'rawModeData': self.raw_mode_data,
